<div>
    <div class="mb-6 flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-semibold text-secondary-900">Manajemen Departemen</h1>
            <p class="text-sm text-secondary-600 mt-1">Kelola departemen perusahaan</p>
        </div>
                ...
                <table class="table">
                    ...
                    <tbody class="divide-y divide-secondary-200">
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="font-medium text-secondary-900">{{ $dept->name }}</div>
                                @if($dept->description)
                                <div class="text-xs text-secondary-500">{{ Str::limit($dept->description, 50) }}</div>
                                @endif
                            </td>
                            <td>{{ $dept->manager?->name ?? '-' }}</td>
                            <td>
                                <span class="badge-primary">{{ $dept->employees->count() }} orang</span>
                            </td>
                            <td>
                                @if($dept->is_active)
                            ...
                            <td>
                                ...
                                <div class="flex items-center space-x-2">
                                    <button wire:click="edit({{ $dept->id }})" class="text-primary-600 hover:text-primary-900">
                                        Edit
                                    </button>
                                    <button wire:click="delete({{ $dept->id }})" 
                                            wire:confirm="Yakin ingin menghapus departemen ini?"
        ...
        <div class="flex min-h-screen items-center justify-center px-4">
            ...
            <div class="relative bg-white rounded-lg max-w-lg w-full p-6">
                ...
                <form wire:submit="save" class="space-y-4">
                    <div>
                        <label class="form-label">Nama Departemen <span class="text-danger-500">*</span></label>
                    ...
                    <div>
                        ...
                        <select wire:model="manager_id" class="form-input">
                    ...
                    <div class="flex items-center">
                        <input wire:model="is_active" type="checkbox" id="is_active" class="h-4 w-4 text-primary-600 focus:ring-primary-500 border-secondary-300 rounded">
                        <label for="is_active" class="ml-2 block text-sm text-secondary-900">
                            Aktif
                        </label>
                    </div>
